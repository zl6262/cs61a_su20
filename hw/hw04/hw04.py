...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y)<0 or lower_bound(y)>0, 'AssertionError'
...
def quadratic(x, a, b, c):
    ...
    delta = -1*b/(2*a)
    if lower_bound(x)< delta and upper_bound(x)>delta:
        return interval(min(f(delta), f(lower_bound(x)), f(upper_bound(x))),
        max(f(delta), f(lower_bound(x)), f(upper_bound(x))))
    else:
