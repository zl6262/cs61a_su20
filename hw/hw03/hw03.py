...
def g_iter(n):
    ...
    "*** YOUR CODE HERE ***"
    i, res = 0, 0
    lst1, lst2, lst3 = 3, 2, 1
    while i<=n:
        if i<=3:
            res = i
        else:
            res = lst1 + 2 * lst2 + 3 *lst3
            lst1, lst2, lst3 = res, lst1, lst2
        i += 1
    return res
...
def missing_digits(n):
    ...
    minus = last-without_last%10
    return max(minus-1,0) + missing_digits(without_last)


def count_change(total):
    """Return the number of ways to m   ake change for total.
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(smallest, total):
        if total == 0:
            return 1
        if smallest > total:
            return 0
        without_coin = helper(smallest*2, total)
        with_coin = helper(smallest, total-smallest)
